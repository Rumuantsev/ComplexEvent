        ...
        saveButton.setOnClickListener {
            if (clarification.isChecked) {
                ...
                if (str.contains("[A-Za-z0-9!\"#$%&'()*+,-./:;\\\\<=>?@\\[\\]^_`{|}~]".toRegex())) {
                    ...
                    val Error = Toast.makeText(this, "Отсутствует текст в поле для ввода", Toast.LENGTH_LONG)
                    Error.show()
                }

            } else {
                val myToast = Toast.makeText(this, "Для сохранения отметьте флажок", Toast.LENGTH_LONG)
                myToast.show()
            }
        }
    }
